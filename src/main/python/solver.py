...
from feed_forward import ArmFeedforward, JointConfig
...
class Solver:
    def __init__(self, config: str, silence: bool = False):
        ...
        elbow_cg_radius = (config["elbow"]["cgRadius"] * config["elbow"]["mass"] + (config["elbow"]["length"])) / (config["elbow"]["mass"])
        elbow_moi = config["elbow"]["mass"] * math.pow(config["elbow"]["cgRadius"] - elbow_cg_radius, 2.0)
    ...
    def _get_motor(self, motor_config) -> DCMotor:
        ...
        elif motor_config["type"] == "falcon500":
