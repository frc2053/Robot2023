...
/**
 * The TimeInterpolatableBufferFIXED provides an easy way to estimate past
 ...
 */
template <typename T>
class TimeInterpolatableBufferFIXED {
 public:
  /**
   * Create a new TimeInterpolatableBufferFIXED.
   *
   * @param historySize  The history size of the buffer.
   * @param func The function used to interpolate between values.
   */
  TimeInterpolatableBufferFIXED(units::second_t historySize,
                           std::function<T(const T&, const T&, double)> func)
      : m_historySize(historySize), m_interpolatingFunc(func) {}

  /**
   * Create a new TimeInterpolatableBufferFIXED. By default, the interpolation
   * function is wpi::Lerp except for Pose2d, which uses the pose exponential.
   *
   * @param historySize  The history size of the buffer.
   */
  explicit TimeInterpolatableBufferFIXED(units::second_t historySize)
...
template <>
WPILIB_DLLEXPORT TimeInterpolatableBufferFIXED<Pose2d>::TimeInterpolatableBufferFIXED(
